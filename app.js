 ...
 console.log(tutorial.PI);
  ...
  /********EVENT MODULES AND EMITTERS***/
   ...
   eventEmitter.on('tutorial',(num1,num2) =>{
    console.log('Tutorial event triggered!! The sum of '+ num1 + ' and ' + num2 + ' is ' , num1 + num2);
  ...
  rl.question(`What is ${num_1} + ${num_2}?\n`,(userInput) =>{
    ...
    if(userInput.trim() == answer){
        ...
        rl.setPrompt('Incorrect response,  Please try again later\n'); /**set the prompt */
        rl.prompt(); /**Call the setPrompt */
        rl.on('line',(userInput) =>{ /**listener for readline interface */ /**line will keep on making it possible for the user to input again until they get it right */
            if(userInput.trim() == answer){
  ...
  rl.on('close',() =>{ /**listen for the user input to verify if it is correct*/

    console.log('You made it buddy! Congz!!\n'); /***Executes the code last, giving final feedback */
  }); 
}, 3000);
  ...
  const fs = require('fs');
  //Create a file
  fs.writeFile('Overlap.txt','This is the general overview of what i studied in this tutorial',(err) =>{
...
/****Rename a file */
fs.rename('test.txt','test.txt',(err,file) =>{ /**test.txt was renamed to test.txt */
   if(err){
...
res.end();//Response is gonna be for anything we type
 ...
 http.createServer((req,res) =>{
  ...
  const readStream_html = fs.createReadStream('./static/index.html' /*,'./package.json'*/);
